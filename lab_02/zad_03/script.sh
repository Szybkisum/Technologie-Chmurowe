...
MONGO_ID=$(docker run -d -p 27017:27017 --name some-mongo mongo:latest)

info "OCZEKIWANIE" "Czekam na uruchomienie MongoDB..."
sleep 5

info "DANE" "Wstawianie testowych danych do MongoDB"
docker exec -i some-mongo mongosh <<EOF
use mydb
...
CONTAINER_ID=$(docker run -d -p 8080:8080 --name node-zad-container -it node:$NODE_VERSION-alpine tail -f /dev/null)
